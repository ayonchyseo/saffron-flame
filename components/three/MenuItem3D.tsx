...
export function MenuItem3D({ category, hovered }: MenuItem3DProps) {
  ...
  return (
    <>
      <ambientLight intensity={0.7} color="#3a2a18" />
      {/* Hemisphere covers sky/ground colour without a render-target */}
      <hemisphereLight intensity={0.8} color="#ffd57a" groundColor="#0d0a08" />
      <directionalLight position={[3, 5, 4]} intensity={1.6} color="#ffd57a" castShadow={false} />
      <directionalLight position={[-3, 2, -2]} intensity={0.5} color="#5b6a8a" />
      <pointLight position={[0, 2, 3]} intensity={0.6} color="#c89b3c" />
  ...
  return (
    <group>
      ...
      <mesh position={[0.28, 0.12, 0.105]} scale={[1, 1, 0.1]}>
        ...
        <meshStandardMaterial color="#ff7a3d" emissive="#ff5b1f" emissiveIntensity={0.5} />
      </mesh>
    </group>
  );
}
